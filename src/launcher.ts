...
export default class ElectronLaunchService implements Services.ServiceInstance {
  ...
  async onPrepare(_: never, capabilities: Capabilities.RemoteCapabilities) {
    ...
    const pkgElectronVersion = (
      dependencies?.electron || devDependencies?.electron ||
      dependencies?.['electron-nightly'] || devDependencies?.['electron-nightly']
    );
...
export async function detectBinaryPath (pkg: NormalizedReadResult, p = process) {
  ...
  const isForgeSetup = Boolean(
    pkg.packageJson.config?.forge ||
    Object.keys(pkg.packageJson.devDependencies || {}).includes('@electron-forge/cli')
  )
  if (isForgeSetup) {
    /**
  ...
  const isElectronBuilderSetup = Boolean(
    pkg.packageJson.build?.appId ||
    Object.keys(pkg.packageJson.devDependencies || {}).includes('electron-builder')
  )
