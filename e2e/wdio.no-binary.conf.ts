...
export const config: WdioElectronConfig = {
  ...
  outputDir: `logs/no-binary-${moduleType}`,
  ...
  before: async () => {
    console.log('🔍 Debug: Starting no-binary test with appEntryPoint:', appEntryPoint);
  },
  // Add after hook to log debug information
  after: async () => {
    console.log('🔍 Debug: Completing no-binary test, cleaning up');
  },
};
